...
2. Validate product IDs, prices, quantities, currency, and success/cancel URLs on the server. Never accept arbitrary amounts from the browser.
3. Start from `templates/cloudflare-pages-checkout.js`; it creates a Checkout Session using `STRIPE_SECRET_KEY` and returns only the session URL.
